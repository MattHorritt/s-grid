...
////	timeBomb();
//
////    printf("In conveyanceFunc, chanMaxD=%f\n",chanMaxD);
//
//	dtm.assign(xsz,ysz,dtmArg);
//
//	profileLength=MAX(abs(xi1-xi0),abs(yi1-yi0))+1;
...
//        chanDepth=MIN(chanWidth/chanAR,chanMaxD);
//
//        nList=new float[profileLength];
//        for(i=0;i<profileLength;i++) nList[i]=manningsN;
//
//        numCellsLower=int(chanWidth/dl);
//
//        if(numCellsLower!=0) printf("Lowering %i cells out of %i\n",numCellsLower,profileLength);
//        fflush(stdout);
//
//        for(i=0;i<numCellsLower;i++)
//        {
//            profile[i]=minZ-chanDepth;
//            nList[i]=nChan;
//        }
//        if(numCellsLower>0) minZ-=chanDepth;
//
//
//
//    }
...
//            {
//                if(channel) mnt=nList[j];
//                else mnt=manningsN;
//			conveyance[i]+=dl*pow(MAX(0.,wl-profile[j]),1.6666667)/mnt;
//            }
//	}
//
...
////    if(maxArea>1000.)
////    {
//////        printf("NaN slope found:\n");
//// 	for(i=0;i<nConveyanceLevels;i++)
////	{
////		wl=minZ+maxDepth*pow(10,(-2.+i*2./(nConveyanceLevels)));
////        printf("wl=%f   c_u_min=%f  c=%f\n",wl,conveyanceUsingMinZ[i],conveyance[i]);
////
////
////
////	}
////
////printf("C++ drainage area=%f width=%f depth=%f\n",maxArea, chanWidth, chanDepth);
////            printf("chanAR=%f chanMaxD=%f\n",chanAR, chanMaxD);
////
////for(j=0;j<profileLength;j++)
////                printf("%f  %f\n", profile[j],nList[j]);
//
...
{
    if(v/1e6>=vip3) return((v/1e6-vip3)+zMax);
    else return(interp4(v/1e6,0,vip1,vip2,vip3,zMin,zMin+1,zMin+5,zMax));
}

float wlFromVol5(float v,float zMin, float zBank, float zMax,
    float vip1, float vip2, float vip3, float vip4)
{
    if(v/1e6>=vip4) return((v/1e6-vip4)+zMax);
    else return(interp5(v/1e6,0,vip1,vip2,vip3,vip4,zMin,zBank,zBank+1,zBank+5,zMax));
}
...
//        else:
//            wl=numpy.interp(v/1e6,[0]+vips,[zMin,zBank,zBank+1,zBank+5,zMax])
//
//    return wl
...
//extern "C" float volFromWL(float wl,
//	float zMin, float zMax, float vip1, float vip2, float vip3)
//{
//	if(wl>=zMax) return(vip3+1e6*(wl-zMax));
//    else return(1e6*interp(wl,zMin,zMin+1,zMin+5,zMax,0,vip1,vip2,vip3));
//}
...
def resample(wl,xll,yll,dx,dtmFileName):
    ...
    depth=numpy.zeros((dtmXsz,dtmYsz))
    wlGrid=numpy.zeros((dtmXsz,dtmYsz))-9999.
