...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def calcStorageParametersChannel(rect,z,xll,yll,dx,\
...
#    print width, depth
    ...
    zBank=cell.min()
    chanVol=width*depth*1000. # Fix this with cellSize
    chanArea=width*1000.
...
def gridFlowSetupTiled(dtmFileName,xll,yll,cellSize,xsz,ysz,nChan,nFP,
    ...
    for i in range(xsz):
        ...
        for j in range(ysz):
            ...
            if xi0>=0 and xi1<dtmXsz and yi0>=0 and yi1<dtmYsz: # Within DTM extent - grab window
                ...
                if nFileName is not None:
                    nWindow=nFileObj.ReadAsArray(xoff=xi0,yoff=dtmYsz-yi1,xsize=windowXsz,ysize=windowYsz).transpose()
                    nWindow=nWindow[:,::-1]
                else:
...
def calcFlow(wl1,wl2,cp):
    ...
    if wl<=zEdge or zEdge==-9999:
        return 0.

    kMin=(dxGrid*((wl-zEdge)**1.666667)/n)
    k=numpy.exp(slope*numpy.log(kMin)+intercept)
...
def calcFlowInertial(Q,wl1,wl2,cp,dt): # Should call by reference
    dxGrid=1000.
    n=0.03
    g=9.81
...
#    print h, zEdge, A
    ...
    flow=(Q-g*S*A*dt)/(1+g*A*dt*abs(Q)/k**2)
#    flowInf=(-g*S*A)/(g*A*abs(Q)/k**2)

    if S>0:
        flowNI=-k*numpy.sqrt(S)
    else:
        flowNI=k*numpy.sqrt(-S)

#unew[i,j]=(u[i,j]/dt-beta*(u[i,j]*dudx+v4*dudy)-g*dhdx)/(1./dt+g*n*n*absVel/(hEdge**(4./3)))

#    if abs(flow)>abs(flowNI) and abs(flow)>1.:
#        print h, A, Q, flow, flowNI

    if numpy.isnan(flow):
#        print h, A, Q, k, flow, flowNI
        flow=flowNI


    return flow

def volFromWl(wl,i,j,sp):
    ...
    if sp.shape[2]==5: # No channel
        ...
        if wl>=zMax:
            v=vips[-1]+1e6*(wl-zMax)
        else:
            v=1e6*numpy.interp(wl,[zMin,zMin+1,zMin+5,zMax],numpy.concatenate(([0],vips)))
        ...
        if wl>=zMax:
            ...
            v=1e6*numpy.interp(wl,[zMin,zBank,zBank+1,zBank+5,zMax],numpy.concatenate(([0],vips)))

    return v

def wlFromVol(v,i,j,sp):
    ...
    if len(sp)==5: # No channel
        ...
        if v/1e6>=vip3: # Cell size
            wl=(v/1e6-vip3)+zMax
        else:
            wl=numpy.interp(v/1e6,[0,vip1,vip2,vip3],[zMin,zMin+1,zMin+5,zMax])
    else:
        zMin=sp[i,j,0]
        zBank=sp[i,j,1]
        zMax=sp[i,j,2]
        ...
        if v/1e6>=vips[-1]:
            wl=(v/1e6-vips[-1])+zMax
        else:
            wl=numpy.interp(v/1e6,numpy.concatenate(([0],vips)),[zMin,zBank,zBank+1,zBank+5,zMax])
    ...
    for iTile in range(numTilesX):
        for jTile in range(numTilesY):
            ...
            fileIO.saveScalarGrid(depthGrid,x0,y0,ddx,\
                outputDirectory+outputPrefix+"_depth_"+tileString+".tif")
            fileIO.saveScalarGrid(wlGrid2,x0,y0,ddx,\
                outputDirectory+outputPrefix+"_wl_"+tileString+".tif")
            ...
            fileIO.saveScalarGrid(lfpGrid,x0,y0,ddx,\
                outputDirectory+outputPrefix+"_lfp_"+tileString+".tif")
            ...
            fileIO.saveScalarGrid(depthGrid,x0,y0,ddx,\
                outputDirectory+outputPrefix+"_merge_"+tileString+".tif")
...
#                arr3=lfpGrid
                ...
                fileIO.saveScalarGrid(wlGrid3,dxll,dyll,ddx,\
                    outputDirectory+outputPrefix+"_wl_fill_"+tileString+".tif")
                ...
                fileIO.saveScalarGrid(depthGrid3,dxll,dyll,ddx,\
                    outputDirectory+outputPrefix+"_depth_fill_"+tileString+".tif")
    ...
    vrtCommand.append(outputDirectory+outputPrefix+'_depth.vrt')

    for iTile in range(numTilesX):
        for jTile in range(numTilesY):
            tileString="tile%02i%02i"%(iTile,jTile)
            vrtCommand.append(outputDirectory+outputPrefix+"_depth_"+tileString+".tif")

    call(vrtCommand)

    # Water levels
    vrtCommand=['gdalbuildvrt']
    vrtCommand.append(outputDirectory+outputPrefix+'_wl.vrt')

    for iTile in range(numTilesX):
        for jTile in range(numTilesY):
            tileString="tile%02i%02i"%(iTile,jTile)
            vrtCommand.append(outputDirectory+outputPrefix+"_wl_"+tileString+".tif")

    call(vrtCommand)

    # Flow paths
    vrtCommand=['gdalbuildvrt']
    vrtCommand.append(outputDirectory+outputPrefix+'_lfp.vrt')

    for iTile in range(numTilesX):
        for jTile in range(numTilesY):
            tileString="tile%02i%02i"%(iTile,jTile)
            vrtCommand.append(outputDirectory+outputPrefix+"_lfp_"+tileString+".tif")

    call(vrtCommand)

    # Merged depth/flow paths
    vrtCommand=['gdalbuildvrt']
    vrtCommand.append(outputDirectory+outputPrefix+'_merge.vrt')

    for iTile in range(numTilesX):
        for jTile in range(numTilesY):
            tileString="tile%02i%02i"%(iTile,jTile)
            vrtCommand.append(outputDirectory+outputPrefix+"_merge_"+tileString+".tif")
    ...
    if extendWlGrid:
        vrtCommand=['gdalbuildvrt']
        vrtCommand.append(outputDirectory+outputPrefix+'_depth_fill.vrt')

        for iTile in range(numTilesX):
            for jTile in range(numTilesY):
                tileString="tile%02i%02i"%(iTile,jTile)
                vrtCommand.append(outputDirectory+outputPrefix+"_depth_fill_"+tileString+".tif")

        call(vrtCommand)

        vrtCommand=['gdalbuildvrt']
        vrtCommand.append(outputDirectory+outputPrefix+'_wl_fill.vrt')

        for iTile in range(numTilesX):
            for jTile in range(numTilesY):
                tileString="tile%02i%02i"%(iTile,jTile)
                vrtCommand.append(outputDirectory+outputPrefix+"_wl_fill_"+tileString+".tif")
    ...
    fileIO.saveVectorCSV(flowX,flowY,xll,yll,cellSize,\
        outputDirectory+outputPrefix+"_flow.csv",thresholdVal=flowThreshold)

    fileIO.saveScalarCSV(wlGrid,xll,yll,cellSize,\
        outputDirectory+outputPrefix+"_wl.csv", headerList=['WL'])
