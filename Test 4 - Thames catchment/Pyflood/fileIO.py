...
def readPolylineShapefile(fileName,midpointsOnly=False):
...
# Return midpoint of line - useful for some BCs
    if midpointsOnly:
...
def readScalarGrid(fileName,dataType=numpy.float64, window=None):
    ...
    if window is None:
        ...
        xOffset=int((xmin-xll)/dx)
        yOffset=int((yll+ysz*dx-ymax)/dx)

        xSize=int((xmax-xmin)/dx)
        ySize=int((ymax-ymin)/dx)

#        print xOffset, yOffset
#        print xSize, ySize

        arr=f.ReadAsArray(xoff=xOffset,yoff=yOffset,xsize=xSize,ysize=ySize).transpose().copy()
        arr=numpy.array(arr[:,::-1],dataType)
