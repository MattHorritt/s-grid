...
outputPrefix="outputs"  # Filename for results is based on this
...
# reservoirShpFileName is polyline of crest (with reservoir to the right), crest level in 
# "cl" attribute, weir coefficient in "wc" attribute
# spillwayShpFileName is points of spillway locations, crest level/width/weir coefficient in
# "cl"/"width"/"wc" attributes
# conveyanceParameterFileName is name of CSV for storing modified conveyance parameters, 
# diagnostic only, can be None

reservoirShpFileName=None
spillwayShpFileName=None
conveyanceParameterFileName=None
...
###########################################################################
if reservoirShpFileName is not None:

    print "Adding reservoirs..."

    spills, fillCellDict, zeroPolyList=reservoirs.modifyConvPar(xll,yll,cellSize,xsz,ysz,convParX,convParY,reservoirShpFileName,\
        csvEdgeFileName=outputDirectory+outputPrefix+"_reservoirEdge.csv")

    # Save these for use in post processing
    fileName=os.path.join(outputDirectory,outputPrefix+"_reservoirs.pck")
    with open(fileName,"w") as f:
        pickle.dump((fillCellDict,zeroPolyList),f)

if spillwayShpFileName is not None:
    print "Adding spillways..."
    reservoirs.spillways(xll,yll,cellSize,xsz,ysz,spills,spillwayShpFileName)

if conveyanceParameterFileName is not None:
    fileIO.saveConveyanceParametersCSV(convParX,convParY,xll,yll,cellSize,conveyanceParameterFileName)
...
while currentTime<(duration*3600.):
    ...
    # Calculate spillway and crest spills
    if reservoirShpFileName is not None:
        reservoirs.calcSpills(wlGrid,spills,flowX,flowY)
