...
parametersFile="params100m.pck"
dtmFileName="dtm.tiff"

outputDirectory="results100m/" # Must have final /
outputPrefix="output"
...
wlShp="wlBCs.shp" # Water level BCs in shapefile - steady state only - use None to turn off
wlAttr='wl'              # Attribute holding water level

# Flow points
flowPointsShp='flowBCs.shp'
flowAttr='flow'
...
saveMax=False # Set to true to save max water levels, flows etc
    ...
    for i in range(xsz):
        for j in range(ysz):
            if storagePar[i,j,0]!=-9999.:
                volGrid[i,j]=sgrid.volFromWl(wlGrid[i,j],i,j,storagePar)
...
while currentTime<(duration*3600.):
    ...
    for wlPt in wlPoints:
        ...
        newV=sgrid.volFromWl(wlPt[2],wlPt[0],wlPt[1],storagePar)
...
if saveEnd:
    ...
    sgrid.saveResults(volGrid,wlGrid,flowX,flowY,storagePar,xsz,ysz,cellSize,xll,yll,
...
#    f.close()
    ...
    sgrid.saveResults(maxVolGrid,wlGrid,maxFlowX,maxFlowY,storagePar,xsz,ysz,cellSize,xll,yll,
                       defaultDepth,flowThreshold,channel,flowPathOutput,
                       tmpFileName,noDataValue,noDataReplacement,
